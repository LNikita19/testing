...
function Login({ setToken }) {
    ...
    const fetchCaptcha = async () => {
        try {
            const response = await axios.get('https://wabasi.sany.in:8991/captcha', { withCredentials: true });
            setCaptcha(response.data.captcha);
        } catch (error) {
            console.error('Error fetching CAPTCHA', error);
        }
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const loginResponse = await axios.post("https://wabasi.sany.in:8991/login", {
                username: username,
                password: password,
                userCaptcha: inputValue
            }, { withCredentials: true });
